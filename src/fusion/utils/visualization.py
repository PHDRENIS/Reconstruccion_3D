...
class Visualizer:
    ...
    def __init__(
        ...
        self.save_overlays_enabled = save_overlays
        self.save_masks_enabled = save_masks
        self.save_json_enabled = save_json
        self.save_depth_enabled = save_depth
    ...
    def draw_detections(
        ...
        for detection in detections:
            ...
            y0 = max(0, bbox[1] - text_size[1] - 8)
            cv2.rectangle(
                result_bgr,
                (bbox[0], y0),
                (bbox[0] + text_size[0] + 6, max(0, bbox[1] - 2)),
                (0, 0, 0),
                -1,
            )
            cv2.putText(
    ...
    def save_overlay(self, result: SemanticResult, split: str = "train") -> str:
        if not self.save_overlays_enabled:
            return ""
    ...
    def save_masks(self, result: SemanticResult, split: str = "train") -> List[str]:
        if not self.save_masks_enabled or not result.masks:
            return []
    ...
    def save_ignore_mask(self, result: SemanticResult, split: str = "train") -> str:
        if not self.save_masks_enabled:
            return ""
    ...
    def save_json(self, result: SemanticResult, split: str = "train") -> str:
        ...
        for i, detection in enumerate(result.detections):
            detection_data = {
                ...
                "bbox": detection.bbox,
                "is_dynamic": detection.class_name.lower() in dynamic_list,
            }
    ...
    def save_depth(
        ...
        if result.depth_gt is not None and self.save_depth_enabled:
            output_path = (
        ...
        if result.depth_input is not None and self.save_depth_enabled:
            output_path = (
