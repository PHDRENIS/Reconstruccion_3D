...
from src.models.yolo_detector import YOLODetector, Detection
...
class SemanticResult:
    """Resultado del pipeline semántico."""
...
class SemanticPipeline:
    """Pipeline completo de procesamiento semántico."""

    def __init__(
        self, config: Dict, device_yolo: str = "cuda", device_rtdetr: str = "cpu"
    ):
        self.config = config
        ...
        self.yolo_seg = YOLOSegmentor(
            ...
            confidence=config.get("yolo", {}).get("confidence_threshold", 0.25),
            iou=config.get("yolo", {}).get("iou_threshold", 0.45),
            device=device_yolo,
        )

        self.rtdetr = None
        self.use_rtdetr = config.get("detector_fusion", {}).get("use_rt_detr", False)
        ...
        self.device_rtdetr = device_rtdetr
