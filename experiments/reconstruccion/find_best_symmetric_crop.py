...
# Calcular centro de masa (mediana es más robusta que media)
center_x = np.median(pts_clean[:, 0])
center_y = np.median(pts_clean[:, 1])

print(f"Centro de masa (mediana): X={center_x:.3f}, Y={center_y:.3f}")

# Probar recortes simétricos alrededor del centro
ranges_to_test = [2.0, 2.5, 3.0, 3.5, 4.0, 5.0, 6.0]
real_width = 3.810
real_length = 4.978
real_height = 2.527
...
for r in ranges_to_test:
    ...
    score = err_w + err_l + err_h
    
    print(f"\nRango ±{r}m desde centro:")
    ...
    if score < best_score:
        best_score = score
        best_result = {
            "method": "symmetric_crop_from_center",
            "range": r,
...
print(f"\n{'='*50}")
print(f"MEJOR RESULTADO: Rango ±{best_result['range']}m desde centro")
...
# Guardar resultados
output_path = Path("C:/Users/renea/Desktop/IPN/TT/Nueva reconstruccion/dimensions_symmetric_best.json")
